...
5. **Cadence / level** — decide whether a message is due (same ladder for both modes):
   ...
   - **Class transitions reset the ladder.** If the class changes between sweeps (e.g. `awaiting_review` → `changes_requested` once a reviewer weighs in, or a new push flips it back), reset `level=1` and `last_nudge_at=-` so the new audience gets a fresh, non-urgent first nudge rather than inheriting the other track's escalation.
...
6. **Compose & send** to the shared Slack channel:
   ...
   - After a successful send, update the row: `nudges += 1`, `last_nudge_at = now`, new `level`, `status = nudging` (reviewer mode) / `nudging-author` (author mode) / `held` at level 4.

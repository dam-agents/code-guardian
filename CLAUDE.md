...
5. **Cadence / level** — decide whether a message is due (same ladder for both modes):
   ...
   - **Anti-spam is critical, and the local `last_nudge_at` guard is NOT sufficient by itself.** Overlapping heartbeats have produced duplicate nudges — PR #2121 got two Slack messages while the ledger showed `nudges=1`, because two runs both read `last_nudge_at=-` and both fired. You MUST follow **Preventing duplicate nudges** below: the claim-before-send push is the authoritative safeguard (just as the remote review marker, not the best-effort in-progress lock, is what truly prevents duplicate reviews). When in doubt, don't send.
6. **Claim, then compose & send** — follow the **Preventing duplicate nudges** protocol below (claim-before-send). In short: record the nudge in `SHEPHERD.md` and push it *first*; only send the Slack message if the push confirms your claim won (no concurrent run already nudged this PR). The message goes to the shared Slack channel:
   ...
   - The row was already advanced by the claim step (`nudges += 1`, `last_nudge_at = now`, new `level`, `status = nudging`/`nudging-author`/`held`). Do not advance it twice.
   - If the Slack send fails *after* a won claim, log it in the chat UI; the row stays advanced (the next run won't re-fire until the cooldown/next tick) — a failed nudge is not a run failure, and under-sending is safer than double-sending.
...
Heartbeats overlap: a slow run can still be sweeping when the next heartbeat starts. Both read the same `SHEPHERD.md`, both see a PR is "due", both post to Slack — and the ledger, written last-write-wins, hides it (PR #2121: two messages, `nudges=1`). The local `last_nudge_at` guard alone cannot prevent this — it only works if reads and writes serialize, which overlapping runs don't. **The work-repo git remote is the one place both runs synchronise, so it is the authority — the same role the remote review marker plays for review dedup.** Four rules, in force for every nudge:

1. **Fetch the freshest ledger before deciding.** At the **start** of the shepherd sweep, `git -C /home/agent/work pull --rebase --autostash origin main`. Never decide "due" from the possibly-hours-stale copy loaded at run start — another run may have already nudged.

2. **Claim-before-send (the authoritative safeguard).** For each PR you judge due, do this **in order**, and only send Slack if the claim wins:
   a. Update the PR's `SHEPHERD.md` row locally: `nudges += 1`, `last_nudge_at = <now, second-precision UTC>`, new `level`, `status`.
   b. Commit just that change and reconcile with the remote: `git -C /home/agent/work commit -m "shepherd: claim nudge PR #<n> L<level>"` then `git pull --rebase --autostash origin main` then `git push origin main`.
   c. **After the rebase, re-read the row.** If a concurrent run already advanced this PR (the rebased row shows a `last_nudge_at` you didn't write, or `nudges` higher than you expected, or a newer `level`), **another run already nudged — abort: do NOT send the Slack message**, and keep the remote's version of the row (don't clobber it). Otherwise your claim is on the remote and is now visible to every other run → **proceed to send**.
   The atomic `git push` is the serialization point: two runs cannot both land a claim for the same PR; the loser sees the winner's row after rebase and backs off. This is heavier than one end-of-run push, but it is the only thing that actually stops cross-pod duplicates — do not skip it for nudges.

3. **Minimum-gap cooldown (belt-and-suspenders).** Never send two nudges for the **same PR** within **`MIN_NUDGE_GAP = 20h`**, no matter what — not for a level escalation, not for a class transition, not for a target change. Since real escalations are ≥2 days apart, this only ever blocks accidental rapid re-sends. Check it *before* the claim: if `now - last_nudge_at < 20h`, skip this PR entirely this sweep.

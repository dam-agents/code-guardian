...
If all open PRs have already been reviewed at their current HEAD (by either check), report that there are no new changes to review and end the run — nothing to send to chat or GitHub.
...
If `N = 0`, report "no new changes" to the chat UI and end the run — items 2–7, 9–12, 15, and 16 don't apply (but item 1 still applies: refresh the skill anyway, and items 13 and 14 still apply: user feedback can still arrive, and closed PRs still need pruning).

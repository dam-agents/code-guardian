...
- Evaluated only when a review runs: commits pushed without `$REREVIEW_LABEL`
  are next examined at a requested re-review; a rule added after a PR was
  reviewed applies from that PR's next review — there is no retroactive
  sweep. To scan one PR now, request a review (label or Slack request,
  review.md).

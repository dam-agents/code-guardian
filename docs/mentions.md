...
**Append the row before any reply, memory write, or review** (write-before-act):
...
`no-action` (combine when several routes ran). The weekly audit trims rows
older than 14 days ([logging.md](logging.md) → **Retention**).
...
2. **Classify and route** (run every route that applies):
   - **Feedback on a review** — a correction, dismissal, disagreement, or
     ...
     was stored.
   - **Question** — answer in a reply: concise, STE, grounded in the PR's
     actual diff and review; when the answer needs data you lack, say what.
...
3. **Ledger row**, then act (reply / memory write / review).
4. **Reply mechanics** — plain comment text, STE style, signed by nothing
   (the account is the signature); no marker line:
...
5. **Log** the terminal state — chain onto the step's last command:
   ...
   A failed POST is retried once, then logged as an error — the ledger row
   stands and the mention is not re-served.

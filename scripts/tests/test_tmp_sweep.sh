#!/usr/bin/env bash
# Stale-clone sweep: reclaim review-pr-* leftovers of dead sessions only — never
# a live lock's dirs, never a fresh clone, never a lookalike name. Runs on every
# heartbeat, not only in the audit.
...
# audit let a week of them pile up.
new_case tmp_sweep_review_mode
base_config
pr_json 7 "open PR" '[]' "7777777777777777777777777777777777777777" | open_prs_fx
add_row 8 "8888888888888888888888888888888888888888" "$(iso_ago 60)" RAPID in_progress
TMP_T="$SANDBOX/tmp"; mkdir -p "$TMP_T"
mkdir -p "$TMP_T/review-pr-9" "$TMP_T/review-pr-8" "$TMP_T/review-pr-10"
touch -t 202001010000 "$TMP_T/review-pr-9" "$TMP_T/review-pr-8"
TMPDIR="$TMP_T" run_preflight review
assert_path gone "$TMP_T/review-pr-9"  'a review run reclaims a dead session clone'

...
# log-session-tokens.sh on SessionEnd, enforce-review-completion.sh on Stop.
...
chmod +x "$SCRIPT" "$TOKENS" "$FINISH" "$ADAPTER_DIR/../../log.sh" 2>/dev/null || true

mkdir -p "$(dirname "$SETTINGS")"
[ -s "$SETTINGS" ] || echo '{}' > "$SETTINGS"

if jq -e --arg c "$SCRIPT" --arg t "$TOKENS" --arg f "$FINISH" \
...
tmp="$(mktemp)"
if jq --arg c "$SCRIPT" --arg t "$TOKENS" --arg f "$FINISH" '
    .hooks //= {} |
  ...
  ' "$SETTINGS" > "$tmp"; then
  mv "$tmp" "$SETTINGS"
  echo "hooks installed into $SETTINGS (PostToolUseFailure + PostToolUse -> $SCRIPT; SessionEnd -> $TOKENS; Stop -> $FINISH)"
else
  rm -f "$tmp"

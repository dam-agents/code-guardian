...
# knows (time-to-first-review, the production model id, the attributed spend,
# check counts).
#
...
JQ_DERIVE='
  ...
  # floor, never a guess. `extras.model` names the production model of the week
  # for events written before the hook recorded one. The audit checks the
  # estimate this produces against `extras.actual_cost_usd` — the spend the
  # platform itself attributed (docs/audit.md task 27).
  def cost:
  ...
  def derive:
    ...
    | { week: .week, ts: .ts, source: (.source // "audit"), version: .definition_version,
        ...
        actual_cost_usd: (.extras.actual_cost_usd // null),
        ...
        pr_files: ($s.project.pr_size.median_files // null),
        pr_lines: ($s.project.pr_size.median_lines // null),
...
write_trends() {
  ...
  [ -f "$DIR/TRENDS.md" ] && markers="$(grep -E '^<!-- audit-trend-(gist|dam): ' "$DIR/TRENDS.md" 2>/dev/null || true)"
  {
    ...
    printf '| week | src | reviews (1st/re) | ✅/⚠️/❌ | new 🔴/🟡/🟢 | f/rev | acc | cov | PR size | human | confl | ttfr | dur | idle | out-tok | est $ | act $ | $/rev | stalled | err/warn |\n'
    printf '|------|-----|------------------|---------|--------------|-------|-----|-----|---------|-------|-------|------|-----|------|---------|-------|-------|-------|---------|----------|\n'
...
if [ "$MODE" = "append" ] || [ "$MODE" = "backfill" ]; then
  ...
  printf '%s' "$DERIVED" | jq -r '
    ...
    | "trend \($c.week): reviews \($c.reviews)\(d($c.reviews; $p.reviews; ""))"
      ...
      + " · coverage \(if $c.coverage == null then "—" else "\($c.coverage)%" end)\(d($c.coverage; $p.coverage; "pp"))"
...
"conflicts": "Count of PRs that hit a merge conflict this week. Lower is better.",
...
"PRs that hit a merge conflict": "Count of PRs that hit a merge conflict in the week. Lower is better.",
...
SUMMARY="$(printf '%s' "$DERIVED" | jq -r --argjson tips "$TIPS" "$JQ_VIEW$JQ_TH"'
  ...
  | if length == 0 then "" else
    ...
    + ([ srow($rows; "Reviews posted"; "reviews"; "up"; "num"),
         ...
         srow($rows; "PRs that hit a merge conflict"; "conflicts"; "down"; "num"),
...
CHARTS="$(printf '%s' "$DERIVED" | jq -r --argjson tips "$TIPS" "$JQ_VIEW"'
  ...
  | if length == 0 then "" else
    ...
    + figure($rows; "Spend per week (USD)"; "Estimate: tokens priced by the CONFIG table, a floor when a model is unpriced. Actual: what the platform attributed, where telemetry covers the week.";
             [{key:"cost_usd", scale:1, color:"var(--c1)", name:"est $ / week"},
              {key:"actual_cost_usd", scale:1, color:"var(--c2)", name:"actual $ / week"}])
...
ROWS_HTML="$(printf '%s' "$DERIVED" | jq -r --argjson tips "$TIPS" "$JQ_VIEW"'
  ...
  | [ to_entries[]
      ...
      | "<tr><td>\($c.week)</td><td>\(if $c.source == "audit" then "audit" else "backfill" end)</td>"
        ...
        + (if $c.coverage == null then "<td class=\"n dash\">—</td>"
           else "<td class=\"n\">\($c.coverage)% of \($c.merged)\(delta($c.coverage; $p.coverage; "up"; "pp"))</td>" end)
        + (if $c.pr_files == null then "<td class=\"n dash\">—</td>"
           else "<td class=\"n\">\($c.pr_files) files / \($c.pr_lines // "—") lines</td>" end)
        + (if $c.human_ttfr_h == null then "<td class=\"n dash\">—</td>"
           else "<td class=\"n\">\($c.human_ttfr_h)h\(rel($c.human_ttfr_h; $p.human_ttfr_h; "down"))</td>" end)
        + ($c.conflicts | cell)
